...
async def handle_link(data, data_size, data_res, update, file_type, context, link):
    global curr_privacy
    global curr_mode
    if (data_res):
        file_name = 'link'
        ...
        await context.bot.send_message(chat_id=update.effective_chat.id, text=f"The file occupies {data_size} MB. EchoBot currently doesn't support files exceeding 25 MB :(")